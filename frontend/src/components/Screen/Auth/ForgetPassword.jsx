...
import { useToast } from '@chakra-ui/react';
 ...
 const ForgetPassword = () =>{
    ...
    const handleForgetButton = (e)=>{
        e.preventDefault()
        dispatch(forgetPasswordAction(email))
    }

    const {isSent ,message, loading:forgetPasswordLoading , error:forgetPasswordError} = useSelector((state)=> state.forgetPassword)

    // useEffect(()=>{

    useEffect(()=>{
        if(forgetPasswordError && forgetPasswordError.length > 0){
            toast({
                title: forgetPasswordError,
                status: "error",
                isClosable: true,
              });
         }
        ...
        <form action="" class="my-10">
            <div class="flex flex-col space-y-5">
                <label for="email">
                    ...
                    <input id="email" name="email" type="email" value={email} onChange={(e)=> setEmail(e.target.value)} class="w-full py-3 border border-slate-200 rounded-lg px-3 focus:outline-none focus:border-slate-500 hover:shadow" placeholder="Enter email address" /> 
                </label>
               {
                forgetPasswordLoading ? <button disabled="" type="button"  class="w-full py-3 font-medium text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg border-indigo-500 hover:shadow inline-flex space-x-2 items-center justify-center">
                ...
                Reset Password
            </button>  : <button onClick={handleForgetButton} class="w-full py-3 font-medium text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg border-indigo-500 hover:shadow inline-flex space-x-2 items-center justify-center">
